...
output "names" {
  ...
  value = [for subnet in azurerm_subnet.this : subnet.name
    if subnet.name != "GatewaySubnet" && subnet.name == "RouteServerSubnet" && subnet.name != "AzureFirewallSubnet" && subnet.name != "AzureFirewallManagementSubnet" && subnet.name != "AzureBastionSubnet" && subnet.name != "RouteServerSubnet"
  ]
}
